...
namespace gbg{
    ...
    Paciente::Paciente(QString nomeCompleto, int peso, float altura, QString sexo):
        ...
        sexo(sexo)
    {
        if (peso <= 0) throw QString("Peso não pode ser <=0");
        if (altura <= 0) throw QString("Altura não pode ser <= 0. Utilize . no lugar de ,");
        this->peso = peso;
        this->altura = altura;
    ...
    QString Paciente::calcularFaixaDePeso()const{
        ...
        if(calcularIMC() <= 35) return QString("Obeseidade");
